...
from PVSource.PVCell.Lookup import Lookup
from solar_car_v2.getweather import cell_temp
from solar_car_v2.getweather import get_irradiance
...
class PVCellNonideal(PVCell):
    ...
    def getCurrent(self, numCells=1, voltage = 0, irradiance=get_irradiance()):
    ...
    def getCurrentLookup(self, numCells=1, voltage=0, irradiance=get_irradiance, temperature=0):
        """
        ...
        """
        return self._lookup.lookup([numCells, voltage, irradiance, temperature])[0]
    
    def getEnergyGen(self, numCells = 1, voltage = 0, irradiance=get_irradiance, temperature = 0, time_period = 1):
        """
        Get energy generated by the array to put into battery model
        """
        current_gen = self.getCurrentLookup(numCells=numCells, voltage=voltage, irradiance=irradiance, temperature=temperature)
        power_gen = current_gen *voltage
        return power_gen * time_period #energy in jouls
