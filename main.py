...
if __name__ == "__main__":
  ...
  env = make_vec_env(lambda render_mode: SolarCarEnv(render_mode), n_envs=n_envs,
  ...
  if args.play:
    ...
    if args.continue_training and not args.new:
      ...
      model = PPO.load(path, env=env, verbose=verbose,
                  tensorboard_log='log', device=device, n_steps=n_steps)
    else:
      print("Starting new training", path)
      model = PPO("MlpPolicy", env, verbose=verbose, tensorboard_log='log', device=device, n_steps=n_steps)

    model.learn(total_timesteps=episode_count*episode_length*n_envs, progress_bar=True)
