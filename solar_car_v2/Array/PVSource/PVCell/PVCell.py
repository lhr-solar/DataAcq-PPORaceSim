...
class PVCell:
    ...
    beta = 0.004 # temperature coefficient (0.004/°C) 
    ...
    def getCurrent(self, numCells=1, irradiance=0.001, temperature=0):
        """
        ...
        SCCurrent = self.refSCCurrent * (irradiance/self.refIrrad) * (1 + self.beta*(temperature - self.refTemp) )
        #takes the rule of thumb into acct --> beta is the temp coefficient (power increases per temp decrease)

        return SCCurrent * numCells #multiply numcells by number of cells
    ...
    def getCurrentLookup(self, numCells=1, voltage=0, irradiance=0.001, temperature=0):
        ...
        float: current of the cell model.
        """
        return self.getCurrent(numCells, irradiance, temperature)
    ...
    def getCellEdgeCharacteristics(
        ...
        """
        mpp = (0, 0)  # voltage, current list, mpp is operating point at which power is maximized
        OCVoltage = 0.0
        ...
        if model != []:
            ...
            for (voltage, current) in model: #just iterating through and finding max power
