...
from .Array.cell import Cell
from .getweather import Weather
from ..solar_car_v2.Array.Array import SolarArray, SolarCell
...
class Battery:
    ...
    """
    pv_model = Array.ThreeParamCell(Cell)

    weather = Weather(0.1)
   
    num_cells = 10  # Number of cells in the array
    power_dc = weather.dc_power()
    voltage = 0.641  # from Pablo
    irradiance = weather.get_irradiance()  # from get wehather
    temperature = 25  # Temperature in Celsius
    time_period = 3600  # Time period in seconds (e.g., 1 hour)
    current_draw = 1.2 #mpp(I) from powergen data
    inverter_efficiency = 0.9 #generally between 0.9-0.98 --> assuming we're not super good...
    motor_efficiency = 0.9 #see inverter_efficiency
    _step = 1
    ...
    def power_into_battery(self):
        array = SolarArray()
        energy = array.step()
        power_into_battery = energy/self.time_step
        return power_into_battery
    ...
    def get_voltage(self) -> float:
        return self.get_pack_voltage() * self.np

    def get_pack_voltage(self) -> float:
        return self._output()["Pack terminal voltage [V]"][-1]
    
    def inverter_conversion(self, power_dc):
        '''
        for the inverter efficency variable in model
        battery converts DC from array to AC for motor
        '''
        return power_dc *self.inverter_efficiency
    
    def output_power(self, inverter_efficiency, motor_efficiency):
        #output_power is power delievered to the motor
        battery_voltage = 60
        battery_current = self.power_into_battery()/battery_voltage
        battery_available_power = battery_voltage * battery_current
        output = battery_available_power * inverter_efficiency * motor_efficiency
        return output
