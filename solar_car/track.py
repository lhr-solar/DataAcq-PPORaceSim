...
def generate_path():
    ...
    points = [
        (
            x + np.random.uniform(-magnitude / 3, magnitude / 3),
            y + np.random.uniform(-magnitude / 3, magnitude / 3),
            z,
        )
        for (x, y, z) in points
    ]
...
def generate_terrain(system, path: chrono.ChBezierCurve):
    ...
    for i, _point in enumerate(path.GetPoints()[1:]):
        for j in np.linspace(0, 1, 100):
            ...
            # normalized tangent to disregard distance
            x, y = deriv.x, deriv.y
